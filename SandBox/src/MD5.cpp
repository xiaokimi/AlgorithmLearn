...
void MD5::Update(const uint8_t* buffer, size_t len)
{
    size_t index = mBitsCount / 8 % 64;
    mBitsCount += len << 3;
    ...
    if (len >= firstPart)
    {
        memcpy(reinterpret_cast<void*>(mBuffer + index), buffer, firstPart);
        Transform(mBuffer);

        for (usedCount = firstPart; usedCount + BLOCK_SIZE <= len; usedCount += BLOCK_SIZE)
        {
            Transform(buffer + usedCount);
        }
    }
    memcpy(reinterpret_cast<void*>(mBuffer + index), buffer + usedCount, len - usedCount);
}
...
void MD5::FinalHash()
{
    ...
    uint8_t bitsCount[8] = { 0 };
    Encode(bitsCount, reinterpret_cast<const uint32_t*>(&mBitsCount), 2);
...
std::string MD5::ToString() const
{
    ...
    for (int i = 0; i < 16; i++)
    {
        sprintf(buffer + i * 2, "%02x", mDigest[i]);
    }
...
void MD5::Decode(uint32_t* output, const uint8_t* input, size_t len)
{
    ...
    {
        *output++ = static_cast<uint32_t>(input[i])
